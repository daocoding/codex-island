...
CodexIsland 是一个原生 macOS 悬浮层，把 MacBook 刘海变成类似 Dynamic Island 的实时用量状态。它支持 Claude Code 和 Codex，用紧凑用量仪表取代装饰性的服务 logo；点击可展开完整面板，查看服务端返回的用量窗口、图表样式，以及从本地会话日志估算的美元成本和 token 吞吐量。
...
- **四项核心用量常驻。** 默认启用“始终显示用量”后，原来的两个 logo 位置会变成紧凑仪表：Claude 外环显示 Fable、中环显示周用量、内环显示 5 小时用量；Codex 使用一个周用量环。两个圆心都显示对应服务的周重置倒计时。常驻状态只占 185pt 物理刘海加左右各 38pt 仪表位；点击后再展开完整详情。
