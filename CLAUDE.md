...
- `Sources/Usage/UsageFetcher.swift` — Codex (`/wham/usage`) and Claude (`/api/oauth/usage`) fetchers. Claude requires the `claude-code/X.Y.Z` User-Agent + `oauth-2025-04-20` beta header. Claude token handling is STRICTLY READ-ONLY (`ClaudeCredentials`): the app never calls the OAuth refresh endpoint and never writes the keychain. Anthropic rotates the refresh token on every refresh call and revokes the whole token family on old-token reuse, so a second refresher racing Claude Code invalidates the user's CLI login (this happened — do not reintroduce refresh). An expired access token is not a signed-out state: the UI waits for the standalone Claude Code CLI to refresh the shared Keychain record, while interactive re-auth is reserved for a genuinely missing scope. Claude Desktop's embedded Code sessions receive a private `CLAUDE_CODE_OAUTH_TOKEN`; a separately launched CodexIsland cannot inherit or safely scrape it. Credential sources, in order: env token → `$CLAUDE_CONFIG_DIR/.credentials.json` (default `~/.claude/.credentials.json`; preferred lazily so a usable file never touches the keychain) → keychain items under "Claude Code-credentials". Unsigned/ad-hoc builds read the latter through Apple's stable `/usr/bin/security` identity so **Always Allow** survives rebuilds; a future certificate-signed build automatically uses in-process `SecItemCopyMatching`. The usage endpoint also requires the `user:profile` scope as of mid-2026 — tokens from older logins return 403 and the only fix is `claude /login`.
