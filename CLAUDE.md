...
| Claude shows an expired-token error beside old week/Fable values | One provider failure is mixed into cached windows with different reset boundaries | Provider health was copied into window errors, expired in-memory windows were not sanitized, and local 5h fallback re-dated unrelated weekly values | Keep `UsageProviderStatus` separate from `WindowUsage`; sanitize every window at runtime; stamp observations per window; local fallback may update only Claude 5h. Access-token expiry is not sign-out: wait for the standalone Claude Code CLI to rotate the shared Keychain record; reserve re-auth UI for missing-scope failures |
| App crashes while creating `UsageStore.shared` | `/usr/bin/security` is launched from the singleton initializer and `waitUntilExit()` pumps the main run loop back into the same `dispatch_once` | Keychain inspection re-enters SwiftUI while the singleton lock is held | Resolve local credential expiry from `refresh()`, after singleton initialization; never invoke the security CLI from a global/static initializer |
...
- `Sources/Model/LaunchAtLoginStore.swift` — owns `SMAppService.mainApp`. Enabled registrations are fingerprinted and repaired only from a durable `/Applications` or `~/Applications` bundle after that binary changes; never register the replaceable `build/CodexIsland.app`. Use `scripts/install-local.sh` for maintained local installs.
- `Sources/Usage/UsageFetcher.swift` — Codex (`/wham/usage`) and Claude (`/api/oauth/usage`) fetchers. Codex reads `~/.codex/auth.json`, the shared store used and automatically refreshed by Codex Desktop's bundled `codex app-server` as well as the standalone CLI; CodexIsland never needs to launch the CLI and remains read-only. Claude requires the `claude-code/X.Y.Z` User-Agent + `oauth-2025-04-20` beta header. Claude token handling is STRICTLY READ-ONLY (`ClaudeCredentials`): the app never calls the OAuth refresh endpoint and never writes the keychain. Anthropic rotates the refresh token on every refresh call and revokes the whole token family on old-token reuse, so a second refresher racing Claude Code invalidates the user's CLI login (this happened — do not reintroduce refresh). An expired access token is not a signed-out state: the UI waits for the standalone Claude Code CLI to refresh the shared Keychain record, while interactive re-auth is reserved for a genuinely missing scope. Claude Desktop's embedded Code sessions receive a private `CLAUDE_CODE_OAUTH_TOKEN`; a separately launched CodexIsland cannot inherit or safely scrape it. Credential sources, in order: env token → `$CLAUDE_CONFIG_DIR/.credentials.json` (default `~/.claude/.credentials.json`; preferred lazily so a usable file never touches the keychain) → keychain items under "Claude Code-credentials". Unsigned/ad-hoc builds read the latter through Apple's stable `/usr/bin/security` identity so **Always Allow** survives rebuilds; a future Developer ID build automatically uses in-process `SecItemCopyMatching`. The usage endpoint also requires the `user:profile` scope as of mid-2026 — tokens from older logins return 403 and the only fix is `claude /login`.
- `Sources/Usage/AppUsage.swift` — owns window observations and provider health. `WindowUsage.observedAt` / `source` describe each reading; `UsageProviderStatus` describes the latest provider attempt independently. Never copy a provider failure into cached windows or advance their observation times. The `plan` field carries Claude's `subscriptionType` (from keychain) or Codex's `plan_type` (from API top-level).
- `Sources/Usage/UsageSnapshotStore.swift` — sanitizes persisted and in-memory readings. A window becomes unknown at its reset boundary (or its reset-less age limit), so cached percentages cannot leak into a new cycle. Claude local-session fallback is five-hour-only and must not re-date weekly/Fable data.
